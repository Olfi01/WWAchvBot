...
namespace WWAchvBot
{
    class Commands
    {
        ...
        public static void StartGame(Update update, string[] args)
        {
            if (update.Message.Chat.Type == ChatType.Supergroup)
            {
                ...
                if (!Game.Games.ContainsKey(chatid))
                {
                    ...
                    Message msg = ReplyToMessage(gamestarttext, update);
                    Message defaultPin = /* Methods.GetPinnedMessage(chatid); */ Pinmessages.FirstOrDefault(x => x.Key == chatid).Value; // Waiting for Telegram.Bot update...

                    if (msg == null) return;
                    Game.Games.Add(chatid, new Game(msg, defaultPin));
                }
        ...
        /// <summary>
        /// Called on admin commands: /pin
        /// </summary>
        public static void SetDefaultPin(Update update, string[] args)
        {
            if (update.Message.Chat.Type != ChatType.Supergroup)
            {
                ReplyToMessage("You must use this command in a group!", update);
                return;
            }
